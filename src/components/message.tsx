...
const Message = ({ message }: MessageProps) => {
  ...
  return (
    <div
      className={`w-full max-w-3xl px-4 mb-4 flex ${isUser ? 'justify-end' : 'justify-start'}`}
    >
      <div
      ...
      >
        <div className="flex flex-col gap-2 p-4">
          <div
            className={`absolute ${isUser ? '-top-3 right-0' : '-top-3 left-0'}`}
          >
          ...
          <div className="flex-1 overflow-hidden mt-2">
            <div
              className={`prose prose-sm md:prose-base w-full break-words dark:prose-invert dark ${isUser ? 'pr-2' : 'pl-2'}`}
            >
              {!isUser && content === null ? (
